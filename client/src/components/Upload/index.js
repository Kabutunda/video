...
function CloudinaryUploadWidget() {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      addVideo({
        variables: {
          title: title, // title from input field
  ...
  return (
    <Container className="upload-container justify-content-md-center">
      ...
      <Row className="justify-content-md-center">
        <button id="upload_widget" onClick={uploadClick} className="btn btn-outline-light btn-sm">
          Upload
        </button>
      </Row>
      <Form>
        <Row className="justify-content-md-center">
          <Col sm={5}>
            <Form.Group as={Col} className="upload-label" controlId="formGridEmail">
              {URL !== '' ? <Form.Label>Video uploaded... Enter title below to submit.</Form.Label> : ''}
              <Form.Control
        ...
        <Row className="justify-content-md-center">
          <button className='btn btn-outline-success btn-sm' type="submit" onClick={handleFormSubmit}>
            Submit
          </button>
        </Row>
      </Form>
